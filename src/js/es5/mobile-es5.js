...
(function (PLUGIN_ID) {
  ...
  */
  fieldCode.areaDrop.forEach(function (_, index) {
    kintone.events.on(['mobile.app.record.create.show', 'mobile.app.record.edit.show', 'mobile.app.record.create.change.' + fieldCode.areaDrop[index], 'mobile.app.record.edit.change.' + fieldCode.areaDrop[index]], function (event) {
      // 地域に対応した都道府県を配列に格納する
      ...
      if (event.type === 'app.record.create.show' || event.type === 'app.record.edit.show') {
        ...
        fieldCode.prefDrop.forEach(function (shownPrefVal) {
          kintone.mobile.app.record.setFieldShown(shownPrefVal, false);
        });
        ...
        kintone.mobile.app.record.getSpaceElement(fieldCode.showPrefSpace[index]).appendChild(div);
      }
